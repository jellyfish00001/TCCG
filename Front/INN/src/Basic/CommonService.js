 ...
 * @returns 
 */
export const getInnPropsalType = async (Year, isReturnPromise = false) => {
    let url = DdlAPIUrl + '/GetInnPropsalType';
    let response = await api.Post(url, JSON.stringify(Year), null, false);
    if (isReturnPromise) {
        return response;
    }
    if (response.ok) {
        let rtn = await response.json();
        rtn.unshift({ text: "請選擇", value: "" })
        return rtn;
    } else {
        return null;
    }
 ...
 * @returns 
 */
export const getInnYear = async () => {
    let url = DdlAPIUrl + '/GetInnYear';
    let response = await api.Post(url, null, null, false);
    if (response.ok) {
        let rtn = await response.json();
        rtn.unshift({ text: "請選擇", value: "" })
        return rtn;
 ...
 * @returns 
 */
export const getUserIfonListByOuId = async (ouId, defaultValue = "", isEnable = true) => {
    let url = DdlAPIUrl + '/GetUserInfoByOrg';
    let form = new FormData();
    form.append('OU_ID', ouId);
    form.append('IS_ENABLE', isEnable);
    let response = await api.Post(url, form, new Headers(), false);
...
* @param {*} identityField 
*/
export const downProjectAttachment = async (identityField ,dbkey) => {
    console.log(identityField,dbkey)
