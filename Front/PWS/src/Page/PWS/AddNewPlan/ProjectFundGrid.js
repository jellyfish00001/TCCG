...
export const ProjectFundGrid = (props) => {
    const { 
        totalMoney,     // 總經費
    ...
    const calculateFunds = () => {
        // 傳入的年度(與原本年度做區分)
        let planYear = parseInt(PLANYEAR);
        let countData = gridChange.map(item => ({ ...item, PLANYEAR: parseInt(item.PLANYEAR) }));
        let beforeMoney = 0;
        let afterMoney = 0;
...
export default ProjectFundGrid;