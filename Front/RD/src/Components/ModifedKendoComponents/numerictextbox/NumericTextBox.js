...
export var NumericTextBox = React.forwardRef(function (props, target) {
  var isAllowValueChange = React.useRef(false)
  var isProcess = React.useRef(false)
  ...
  var valueGetter = React.useCallback(
    function () {
  ...
  var nameGetter = React.useCallback(
    function () {
      return props.name
    },
    [props.name]
  )
  var requiredGetter = React.useCallback(
    function () {
      return props.required
    },
    [props.required]
  )
  var validityGetter = React.useCallback(
    function () {
  ...
  var validityStylesGetter = React.useCallback(
    function () {
      return props.validityStyles
    },
    [props.validityStyles]
  )
  var propsGetter = React.useCallback(
    function () {
      return props
    },
    [props]
  )
  var getImperativeHandle = React.useCallback(
    function () {
      var options = {
  ...
  var triggerChange = React.useCallback(
    function (event, newState) {
      if (props.disabled) {
        return
      }
      if (isAllowValueChange.current) {
        if (isProcess.current) {
          newState = prevState.current;
          isProcess.current = false
          prevState.current = null;
          isAllowValueChange.current = false
        }
      }
      ...
      if (newState.valueIsCorrected) {
        ...
        if (
          wrapper_1 &&
          wrapper_1.className.indexOf('k-state-invalid') === -1
        ) {
          wrapper_1.className += ' k-state-invalid'
          window.setTimeout(function () {
            return (wrapper_1.className = wrapper_1.className.replace(
              ' k-state-invalid',
              ''
            ))
          }, 50)
  ...
  var onChangeHandler = React.useCallback(
    function (event) {
      // if(isAllowValueChange.current){
  ...
  var onKeyDown = React.useCallback(
    function (event) {
      ...
      if (event.key !== 'Process') {
        isProcess.current = false
        isAllowValueChange.current = true
      } else {
      ...
      var currentValue = intlService.parseNumber(
        ...
        case 110:
          ...
          if (element) {
            newState.currentLooseValue =
              newState.currentLooseValue.slice(0, newState.selectionStart) +
              symbols.decimal +
  ...
  var onIncrease = React.useCallback(
    function (event) {
      var newState = getCurrentState()
      increaseValue(
  ...
  var onDecrease = React.useCallback(
    function (event) {
      var newState = getCurrentState()
      decreaseValue(
        intlService.parseNumber(
  ...
  var onWheelHandler = React.useCallback(
    function (event) {
      if (
        !document ||
  ...
  var numerictextbox = React.createElement(
    'span',
    {
      dir: props.dir,
      style: !props.label ? { width: props.width } : undefined,
      className:
    ...
    React.createElement(
      ...
      React.createElement(
        'span',
        { className: 'k-select', onMouseDown: onMouseDown },
        React.createElement(
          'span',
          {
