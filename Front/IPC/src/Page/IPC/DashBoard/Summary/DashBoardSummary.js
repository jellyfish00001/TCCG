...
const DashBoardSummary = (props) => {
    ...
    const [orgSummary, setOrgSummary] = useState({
        ...
        BudgetTotalStr: 0,             // 總經費
        BudgetTotalDiffStr: 0,         // 總經費較上月差異
        DelayCount: 0,              // 落後件數
    ...
    const sortAndTopN = (objArray, propName, isDescending, fetchNum) => {
        ...
        return objArray.sort((a, b) => {
            const valueA = parseFloat(a[propName]);
            const valueB = parseFloat(b[propName]);

            // 首先按 propName 進行排序
            if (valueA === valueB) {
                // 如果 value 相等，則按 SET_VALUE 文字順序排序
                return a.SET_VALUE.localeCompare(b.SET_VALUE);
            }
    ...
    const getOrgSummary = async () => {
        ...
        if (orgSummary) {
            ...
            setOrgSummary({
                ...orgSummary,
                E1DelayCount: categoryDelayData.filter(x => x.PROJECT_CATEGORY == "E1" || x.PROJECT_CATEGORY == 'S1').length,
    ...
    return (
        ...
        >
            <div className={style.titleBar} style={{ display: 'flex', alignItems: 'center' }}>
                ...
                <div >
                    <div className={style.description}>
                        總經費 <span className={style.highlights}>{orgSummary.BudgetTotalStr}</span> 億元
                        <br />
                        (較上月{formatDiff(orgSummary.BudgetTotalDiffStr)}億元)
                    </div>
                </div>
                <div style={{ marginLeft: 'auto' }}>
                    <span
                        onClick={() => getStatisticsRPT(statCycle.DAB_YEAR_YYY, statCycle.DAB_MONTH, 1)}
                ...
                <div style={{ position: 'relative' }}>
                    <div className={style.chartTitleContainer}>
                        <div className={style.chartTitle}>
                            {statType == 'count' ? '各行政區列管數' : '各行政區列管經費'}
                        </div>
                        <div className={style.chartTitleCell}>
                            {'單位：件/億元'}
                        </div>
                    </div>
            ...
            <div className={style.titleBar} style={{ height: '100px', display: 'flex', alignItems: 'center' }}>
                ...
                <div style={{ display: "inline-flex", minWidth: "600px" }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center' }}>
                        <ul>
                            ...
                            <li>
                                連續落後三個月以上案件比率{orgSummary.DelayOver3MonthsRate}%
                ...
                <div style={{ marginLeft: 'auto' }}>
                    <span
                        onClick={() => getStatisticsRPT(statCycle.DAB_YEAR_YYY, statCycle.DAB_MONTH, 2)}
                        className={style.reportLink}
                        style={{ backgroundColor: '#409BCA' }}
                    >
                        落後情形報表
            ...
            <div style={{
                ...
                < div style={{ marginLeft: 'auto' }}>
                    <span
                        onClick={() => getStatisticsRPT(statCycle.DAB_YEAR_YYY, statCycle.DAB_MONTH, 4, "Statistics")}
                        className={style.reportLink}
                    >
                        落後排名報表
                    </span>
                ...
                <div style={{ marginLeft: 'auto', marginTop: '10px' }}>
                    <span
                        onClick={() => getStatisticsRPT(statCycle.DAB_YEAR_YYY, statCycle.DAB_MONTH, 5, "Delay")}
                        className={style.reportLink}
                    >
                        落後案件清單
                    </span>
