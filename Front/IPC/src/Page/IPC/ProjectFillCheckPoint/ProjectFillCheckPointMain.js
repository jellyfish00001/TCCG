...
export const ProjectFillCheckPointMain = (props) => {
    ...
    const SaveData = async (data) => {
        ...
        if (type === "adjust") {
            ...
            // 找到 editType 為 1 的資料行
            let editTypeData = sortedDataForSave.find(x => x.editType === 1);
            // 如果有找到 editType 為 1 的資料行，則進行後續的比較
            if (editTypeData) {
                let newProgress = editTypeData.PROGRESS;
                // 找到已填報實際竣工日的檢核點的進度
                let maxActualEnddate = Math.max(...sortedDataForSave.filter(x => x.ACTUAL_ENDDATE != null).map(x => x.PROGRESS));
                // 找到開工檢核點的進度
                if (cpKind == "0") {
                    let maxStartProgress = Math.max(...sortedDataForSave.filter(x => x.CTRL_POINT === "A").map(x => x.PROGRESS));
                    if (newProgress > maxStartProgress) {
                        showGlobalMessageBox("檢核點進度不可大於開工檢核點進度");
                        return;
                    }
                }
                let maxStartProgress = Math.max(...sortedDataForSave.filter(x => x.CTRL_POINT === "A").map(x => x.PROGRESS));
                // 如果maxStartProgress為空值，則不進行檢核
                if (maxStartProgress != "-Infinity") {
                    // 新增要小於開工且大於有實際日期的檢核點
                    if (newProgress < maxActualEnddate || newProgress > maxStartProgress) {
                        showGlobalMessageBox("檢核點進度不可大於已填報實際竣工日的檢核點進度");
                        return;
                    }
                }
