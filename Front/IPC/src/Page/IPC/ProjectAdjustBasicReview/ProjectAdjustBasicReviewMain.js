...
import { IsNullOrEmpty, SetMaskOnOff, closeAndbackToParentWindow } from '../../../Basic/SDOExtension';
...
import { initData, getPageData, saveAuditReview, getZip } from './ProjectAdjustBasicReviewService';
...
const ProjectAdjustBasicReviewMain = (props) => {
    const {
        location: {
            state
        },
    ...
    const displayReasons = () => {
        ...
        return (
            editedFormData.Reasons.map(reason => {
                const text = adjustReasons.find(item => item.SET_TYPE == reason.SET_TYPE).SET_VALUE;
                return (
    ...
    return (
        ...
        } title="基本資料調整審核" isFirstArea={true} >
            <form>
                {isShowBtn && <div className="fn-buttons">
                    <Button title="確認送出" type='button' onClick={() => save(1)} >確認送出</Button>
                    <Button title="調整內容" type='button' className="k-button-lighten" onClick={getAdjustContent}>調整內容</Button>
                </div>}
                <table>
                    <tbody>
                        <tr>
...
export default ProjectAdjustBasicReviewMain;