import * as React from 'react';
import { Checkbox } from '@progress/kendo-react-inputs';
import { IsNullOrEmpty, IsFunction } from '../../Basic/SDOExtension'
import '../../Css/CheckBoxList.css'

///共用CheckBoxList
///Param:
/// -group(string):群組ID
/// -groupName(string)?:群組名稱 ps:不為空值則顯示Title
/// -data(array<object>):設定資料。 ps:checked欄位可預設是否勾選
/// -onChange(function):onChange事件
/// -valueField(string):value欄位名稱
/// -textField(string):text欄位名稱
const CheckBoxList = props => {
    let title = null;
    if (!IsNullOrEmpty(props.groupName)) {
        title =
            <li key={props.group + '_title'}>
                <label style={{
                    borderBottom: '1px solid darkgray',
                    minWidth: '240px',
                    margin: '2px 0 5px 0'
                }}>
                    {props.groupName}
                </label>
            </li>
    }

    const onChange = (event, dataItem) => {
        if (IsFunction(props.onChange)) {
            event['dataItem'] = dataItem;
            props.onChange(event);
        }
    }

    return (
        <ul className="checkboxList">
            {title}
            {props.data.map(data =>
                <li key={props.group + '_' + data[props.textField]} style={{ display: 'inline', whiteSpace: 'nowrap' }}>
                    <span style={{ whiteSpace: 'nowrap' }}>
                        <Checkbox
                            name={props.group + '_' + data[props.valueField]}
                            value={data[props.valueField]}
                            defaultChecked={data.checked}
                            disabled={data.disabled}
                            onChange={event => onChange(event, data)}
                        />
                        &nbsp;{data[props.textField]}
                    </span>
                </li>
            )}
        </ul>
    )
}

export default React.memo(CheckBoxList)