...
namespace SDO.APP.RPT.Excel
{
    ...
    /// 表3：每月案件機關統計表 簡版
    /// </summary>
    public class RPTIPCProjectDeptShort : XlsBuilder
    {
        ...
        // 原始SQL資料
        List<ProjectAreaDeptShortModel> statisticsData;
        // 機關資料
        List<ProjectAreaDeptShortModel> execOrganData = new();
        StatisticsModel model;

        public RPTIPCProjectDeptShort(IComponentContext coms) : base(coms)
        {
            TemplateFileName = "IPCProjectDeptShortRPT.xls";
        ...
        protected override async Task GetData()
        {
            ...
            statisticsData = await statisticsDac.GetIPCProjectAreaDeptShort(model);
        ...
        /// </summary>
        /// <returns></returns>
        private Dictionary<string, string> GetExecOrganDic()
        {
            Dictionary<string, string> result = statisticsData.Select(x => new { x.EXEC_ORGAN_C, x.EXEC_DEPT }).Distinct()
        ...
        {
            foreach (KeyValuePair<string, string> item in GetExecOrganDic())
            {
                IEnumerable<ProjectAreaDeptShortModel> data = statisticsData.Where(x => x.EXEC_ORGAN_C == item.Key);
                int organNum = data.Count();
                int organDelayNum = data.Where(x => x.STATUS.StartsWith("D")).Count();
                execOrganData.Add(new ProjectAreaDeptShortModel
                {
                    EXEC_ORGAN_C = item.Key,
                    EXEC_DEPT = item.Value,
                    NUM = organNum,
                    EXS = Math.Round((double)data.Sum(x => x.PROJECT_EXS) / 100000000, 1),
                    DELAY_NUM = organDelayNum,
                    OU_SORT_ORDER = data.Select(x => x.OU_SORT_ORDER).FirstOrDefault()
                });
            }
            switch (model.SORT_TYPE_3)
            {
                case "B": //依機關群組
                    execOrganData = execOrganData.OrderBy(x => x.OU_SORT_ORDER).ToList();
                    break;
                case "C": //列管件數
                    execOrganData = execOrganData.OrderByDescending(x => x.NUM).ToList();
                    break;
                case "D": //總金額
                    execOrganData = execOrganData.OrderByDescending(x => x.EXS).ToList();
                    break;
            }
        }
        ...
        protected override void MakeContent()
        {
            ...
            CellReplaceByExcel(new
            {
                ...
                SPEC_NOTE = model.SPEC_NOTE.Count == 0
                    ? "重大建設計畫B級管制"
                    : string.Join("、", specNoteDatas.Where(x => model.SPEC_NOTE.Contains(x.SET_TYPE)).Select(x => x.SET_VALUE).ToArray())
            });
            ...
            // 資料
            SetData();
            // 刪除 #EXEC_DEPT#(#PROJECT_NUM#) 樣板
            Xls.Worksheets.RemoveAt("#EXEC_DEPT#(#PROJECT_NUM#)");
        }
        ...
        private void SetStatBriefTable()
        {
            ...
            CellReplaceBySheet(new
            {
                ...
                TOTAL_NUM = sumA,
                NEW_NUM = statisticsData.Where(x => x.PROJECT_YEAR == int.Parse(model.PROJECT_YEAR)).Count(),
                BEFORE_YEAE = int.Parse(model.STATISTICS_YEAR) - 1,
                BEFORE_NUM = $"{(model.PROJECT_YEAR_STATUS == "B" ? "持續列管" : "")}{statisticsData.Where(x => x.PROJECT_YEAR <= int.Parse(model.PROJECT_YEAR) - 1).Count()}",
                CLOSE_NUM = sumB,
                CLOSE_RATE = rateB,
                CONFORM_NUM = sumC,
                CONFORM_RATE = rateC,
            ...
            int row = strRow;
            int index = 1;
            foreach (ProjectAreaDeptShortModel item in execOrganData)
            {
                if (index > 1)
                {
                ...
                SetColumn(cells[$"A{row}"], item.EXEC_DEPT);
        ...
        private void SetStatGeneralTable()
        {
            ...
            int row = strRow;
            int index = 1;
            foreach (ProjectAreaDeptShortModel item in statisticsData)
            {
                if (index > 1)
                {
                    cells.InsertRow(row - 1);
                ...
                SetColumn(cells[$"C{row}"], item.MASTER_DEPT);
                SetColumn(cells[$"D{row}"], item.EXEC_DEPT);
                SetColumn(cells[$"E{row}"], item.TOWNNAME);
                SetColumn(cells[$"F{row}"], item.PROJECT_EXS);
                SetColumn(cells[$"G{row}"], item.ACT_START.HasValue
                    ? $"{item.ACT_START.ToTwDateString()}\n已開工"
                    : item.EST_START.ToTwDateString());
                SetColumn(cells[$"H{row}"], item.ACT_COM.HasValue
                    ? $"{item.ACT_COM.ToTwDateString()}\n已竣工"
                    : item.EST_COM.ToTwDateString());
                SetColumn(cells[$"I{row}"], item.PROJECT_STATUS == "8" || item.PROJECT_STATUS == "7"
            ...
            int endRow = index > 1 ? row - 1 : row;
            int subtotalRow = index > 1 ? row : row + 1;
            SetFormula(cells[$"F{subtotalRow}"], $"=SUM(F{strRow}:F{endRow})");
        }
        ...
        private void SetData()
        {
            foreach (ProjectAreaDeptShortModel item in execOrganData)
            {
                IEnumerable<ProjectAreaDeptShortModel> data = statisticsData.Where(x => x.EXEC_ORGAN_C == item.EXEC_ORGAN_C);
                List<ProjectAreaDeptShortModel> cp0Data = data.Where(x => x.CP_KIND == "0").ToList();
                List<ProjectAreaDeptShortModel> cp1Data = data.Where(x => x.CP_KIND == "1").ToList();

                Sheet = Xls.Worksheets[Xls.Worksheets.AddCopy("#EXEC_DEPT#(#PROJECT_NUM#)")];
                Sheet.Name = $"{item.EXEC_DEPT}({cp0Data.Count + cp1Data.Count})";

                CellReplaceBySheet(new
                {
                    EXEC_DEPT = item.EXEC_DEPT,
                ...
                foreach (ProjectAreaDeptShortModel cp0 in cp0Data)
                {
                    if (cp0Index > 1)
                    {
                    ...
                    SetColumn(cells[$"C{cp0Row}"], cp0.TOWNNAME);
                    SetColumn(cells[$"D{cp0Row}"], cp0.PROJECT_EXS);
                    SetColumn(cells[$"E{cp0Row}"], cp0.ACT_START.HasValue
                        ? $"{cp0.ACT_START.ToTwDateString()}\n已開工"
                        : cp0.EST_START.ToTwDateString());
                    SetColumn(cells[$"F{cp0Row}"], cp0.ACT_COM.HasValue
                        ? $"{cp0.ACT_COM.ToTwDateString()}\n已竣工"
                        : cp0.EST_COM.ToTwDateString());
                    SetColumn(cells[$"G{cp0Row}"], cp0.PROJECT_STATUS == "8" || cp0.PROJECT_STATUS == "7"
                ...
                foreach (ProjectAreaDeptShortModel cp1 in cp1Data)
                {
                    if (cp1Index > 1)
                    {
                    ...
                    SetColumn(cells[$"C{cp1Row}"], cp1.TOWNNAME);
                    SetColumn(cells[$"D{cp1Row}"], cp1.PROJECT_EXS);
                    SetColumn(cells[$"E{cp1Row}"], $"{cp1.EST_START.ToTwDateString()}{(cp1.ACT_START.HasValue ? "\n已開工" : "")}");
                    SetColumn(cells[$"F{cp1Row}"], $"{cp1.EST_COM.ToTwDateString()}{(cp1.ACT_COM.HasValue ? "\n已竣工" : "")}");
                    SetColumn(cells[$"G{cp1Row}"], cp1.PROJECT_STATUS == "8" || cp1.PROJECT_STATUS == "7"
                ...
                // 合計
                SetColumn(cells[$"D{ cp1Row }"], (cp0Data.Sum(x => x.PROJECT_EXS) + cp1Data.Sum(x => x.PROJECT_EXS)));
            }
        }
