...
namespace SDO.APP.RPT.Excel
{
    ...
    /// 儀表板表12：每月案件地區統計表 詳版
    /// </summary>
    public class RPTDABAreaDetail : XlsBuilder
    {
        ...
        public RPTDABAreaDetail(IComponentContext coms) : base(coms)
        {
        ...
        private void SetOrderData()
        {
            ...
            // 篩選掉areaDict中H00和H01的資料
            areaDict = areaDict.Where(x => x.Key != "H00" && x.Key != "H01").ToDictionary(x => x.Key, y => y.Value);
        ...
        private void SetStatGeneralTable()
        {
            ...
            // 施工中
            List<ProjectAreaDeptDetailedModel> underConstructionData = statisticsData.Where(x => x.ACT_START.HasValue && !x.ACT_COM.HasValue).ToList();
            // 尚未開工
            ...
            // 施工中
            currentRow = AddDataToSheet(cells, statisticsData.Where(x => x.ACT_START.HasValue && !x.ACT_COM.HasValue).ToList(), currentRow);
        ...
        private void SetData()
        {
            foreach (ProjectAreaDeptDetailedModel item in areaData)
            {
                ...
                isAnyValue = false;
                // 施工中
                List<ProjectAreaDeptDetailedModel> underConstructionData = data.Where(x => x.ACT_START.HasValue && !x.ACT_COM.HasValue).ToList();
                // 尚未開工
