...
namespace SDO.APP.RPT.Excel
{
    ...
    /// 表3：每月案件機關統計表 詳版
    /// </summary>
    public class RPTProjectDeptDetailed : XlsBuilder
    {
        ...
        // 機關資料
        List<ProjectAreaDeptDetailedModel> execOrganData = new();
        StatisticsModel model;

        public RPTProjectDeptDetailed(IComponentContext coms) : base(coms)
        {
            TemplateFileName = "IPCProjectDeptDetailedRPT.xls";
        ...
        private void SetOrderData()
        {
            ...
            {
                IEnumerable<ProjectAreaDeptDetailedModel> data = statisticsData.Where(x => x.EXEC_ORGAN_C == item.Key);
                int organNum = data.Count();
                int organDelayNum = data.Where(x => x.STATUS.StartsWith("D")).Count();
                execOrganData.Add(new ProjectAreaDeptDetailedModel
                {
        ...
        private void SetStatBriefTable()
        {
            ...
            int row = strRow;
            int index = 1;
            foreach (ProjectAreaDeptDetailedModel item in execOrganData)
            {
                if (index > 1)
                {
        ...
        private void SetData()
        {
            foreach (ProjectAreaDeptDetailedModel item in execOrganData)
            {
                IEnumerable<ProjectAreaDeptDetailedModel> data = statisticsData.Where(x => x.EXEC_ORGAN_C == item.EXEC_ORGAN_C);
                ...
                foreach (ProjectAreaDeptDetailedModel cp0 in cp0Data)
                {
                    ...
                    SetColumn(cells[$"E{cp0Row}"], cp0.TOWNNAME);
                    SetColumn(cells[$"F{cp0Row}"], cp0.PROJECT_EXS);
                    SetColumn(cells[$"G{cp0Row}"], cp0.ACT_START.HasValue
                        ? $"{cp0.ACT_START.ToTwDateString()}\n已開工"
                        : cp0.EST_START.ToTwDateString());
                    SetColumn(cells[$"H{cp0Row}"], cp0.ACT_COM.HasValue
                        ? $"{cp0.ACT_COM.ToTwDateString()}\n已竣工"
                        : cp0.EST_COM.ToTwDateString());
                    SetColumn(cells[$"I{cp0Row}"], $"預定：\n{cp0.RES_CHECKITEM}\n實際：\n{cp0.ACT_CHECKITEM}");
                    SetColumn(cells[$"J{cp0Row}"], $"預定：{cp0.IPC_RES_PRG:N1}\n實際：{cp0.IPC_ACT_PRG:N1}\n差異：{(cp0.IPC_ACT_PRG - cp0.IPC_RES_PRG):N1}%");
                    SetColumn(cells[$"K{cp0Row}"], cp0.EXECUTE_CONDITION);
                    SetColumn(cells[$"L{cp0Row}"], cp0.DELAY_CAUSAL);
                    SetColumn(cells[$"M{cp0Row}"], cp0.DELAY_TYPE);
                    SetColumn(cells[$"N{cp0Row}"], cp0.PROJECT_STATUS == "8" || cp0.PROJECT_STATUS == "7"
                ...
                foreach (ProjectAreaDeptDetailedModel cp1 in cp1Data)
                {
                    ...
                    SetColumn(cells[$"E{cp1Row}"], cp1.TOWNNAME);
                    SetColumn(cells[$"F{cp1Row}"], cp1.PROJECT_EXS);
                    SetColumn(cells[$"G{cp1Row}"], $"{cp1.EST_START.ToTwDateString()}{(cp1.ACT_START.HasValue ? "\n已開工" : "")}");
                    SetColumn(cells[$"H{cp1Row}"], $"{cp1.EST_COM.ToTwDateString()}{(cp1.ACT_COM.HasValue ? "\n已竣工" : "")}");
                    SetColumn(cells[$"I{cp1Row}"], $"預定:{cp1.CHECKITEM_NAME}{cp1.ESTIMATED_ENDDATE.ToTwDateString()}\n實際:{cp1.CHECKITEM_NAME}{cp1.ACTUAL_ENDDATE.ToTwDateString()}");
                    SetColumn(cells[$"J{cp1Row}"], $"預定:{cp1.IPC_RES_PRG:N1}\n實際:{cp1.IPC_ACT_PRG:N1}\n差異:{(cp1.IPC_ACT_PRG - cp1.IPC_RES_PRG):N1}%");
                    SetColumn(cells[$"K{cp1Row}"], cp1.EXECUTE_CONDITION);
                    SetColumn(cells[$"L{cp1Row}"], cp1.DELAY_CAUSAL);
                    SetColumn(cells[$"M{cp1Row}"], cp1.DELAY_TYPE);
                    SetColumn(cells[$"N{cp1Row}"], cp1.PROJECT_STATUS == "8" || cp1.PROJECT_STATUS == "7"
                ...
                // 合計
                SetColumn(cells[$"F{ cp1Row }"], (cp0Data.Sum(x => x.PROJECT_EXS) + cp1Data.Sum(x => x.PROJECT_EXS)));
            }
        }
