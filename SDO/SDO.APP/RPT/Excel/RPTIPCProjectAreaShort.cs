...
namespace SDO.APP.RPT.Excel
{
    ...
    /// 表2：每月案件地區統計表 簡版
    /// </summary>
    public class RPTIPCProjectAreaShort : XlsBuilder
    {
        ...
        // 地區資料
        List<ProjectAreaDeptShortModel> areaData = new();
        StatisticsModel model;
        public RPTIPCProjectAreaShort(IComponentContext coms) : base(coms)
        {
            TemplateFileName = "IPCProjectAreaShortRPT.xls";
        ...
        private void SetOrderData()
        {
            ...
            {
                List<ProjectAreaDeptShortModel> data = statisticsData.Where(x => x.TOWN_C == item.Key).ToList();
                ...
                areaData.Add(new ProjectAreaDeptShortModel
                {
                    TOWN_C = item.Key,
                    TOWNNAME = item.Value,
                    NUM = areaNum,
        ...
        private void SetStatBriefTable()
        {
            ...
            int row = strRow;
            int index = 1;
            foreach (ProjectAreaDeptShortModel item in areaData)
            {
                if (index > 1)
                {
        ...
        private void SetData()
        {
            foreach (ProjectAreaDeptShortModel item in areaData)
            {
                IEnumerable<ProjectAreaDeptShortModel> data = statisticsData.Where(x => x.TOWN_C == item.TOWN_C);
                ...
                foreach (ProjectAreaDeptShortModel cp0 in cp0Data)
                {
                    ...
                    SetColumn(cells[$"C{cp0Row}"], cp0.MASTER_DEPT);
                    SetColumn(cells[$"D{cp0Row}"], cp0.EXEC_DEPT);
                    SetColumn(cells[$"E{cp0Row}"], cp0.PROJECT_EXS);
                    SetColumn(cells[$"F{cp0Row}"], cp0.ACT_START.HasValue
                        ? $"{cp0.ACT_START.ToTwDateString()}\n已開工"
                        : cp0.EST_START.ToTwDateString());
                    SetColumn(cells[$"G{cp0Row}"], cp0.ACT_COM.HasValue
                        ? $"{cp0.ACT_COM.ToTwDateString()}\n已竣工"
                        : cp0.EST_COM.ToTwDateString());
                    SetColumn(cells[$"H{cp0Row}"], cp0.PROJECT_STATUS == "8" || cp0.PROJECT_STATUS == "7"
                ...
                foreach (ProjectAreaDeptShortModel cp1 in cp1Data)
                {
                    ...
                    SetColumn(cells[$"C{cp1Row}"], cp1.MASTER_DEPT);
                    SetColumn(cells[$"D{cp1Row}"], cp1.EXEC_DEPT);
                    SetColumn(cells[$"E{cp1Row}"], cp1.PROJECT_EXS);
                    SetColumn(cells[$"F{cp1Row}"], $"{cp1.EST_START.ToTwDateString()}{(cp1.ACT_START.HasValue ? "\n已開工" : "")}");
                    SetColumn(cells[$"G{cp1Row}"], $"{cp1.EST_COM.ToTwDateString()}{(cp1.ACT_COM.HasValue ? "\n已竣工" : "")}");
                    SetColumn(cells[$"H{cp1Row}"], cp1.PROJECT_STATUS == "8" || cp1.PROJECT_STATUS == "7"
                ...
                // 合計
                SetColumn(cells[$"E{ cp1Row }"], (cp0Data.Sum(x => x.PROJECT_EXS) + cp1Data.Sum(x => x.PROJECT_EXS)));
            }
        }
