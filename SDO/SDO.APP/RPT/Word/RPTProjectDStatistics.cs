...
namespace SDO.APP.RPT.Word
{
    ...
    /// 表1 每月案件統計表
    /// </summary>
    public class RPTProjectDStatistics : WordBuilder
    {
        public RPTProjectDStatistics(IComponentContext coms) : base(coms)
        {
        ...
        protected override async Task<bool> MakeContent()
        {
            ...
            switch (Parameter.Type)
            {
                case "Statistics": // 案件統計表
                    await CreateService<RPTProjectStatistics>(objData, "ProjectDStatistics").MakeContenByTemplate();
                    break;
                case "Status": // 案件狀態表
                case "Delay": // 落後案件表
                    ...
                    await CreateService<RPTProjectStatistics>(objData, "ProjectDStatus").MakeContent();
                    break;
            }
