...
{
    public class IPCProjectADJDetailedCheckpoint : WContentBuilder
    {
        public IPCProjectADJDetailedCheckpoint(IComponentContext coms) : base(coms)
        {
            TemplateFileName = "IPCProjectADJDetailedRPTCheckpoint.doc";
        }
        ...
        {
            List<AdjustCusCheckPointModel> adjustCheckPoints = data.AdjustCusChkpt ?? new();

            List<object> checkpointList = new();
            foreach (AdjustCusCheckPointModel adjustCheckPoint in adjustCheckPoints)
            {
                checkpointList.Add(new
                {
                    CHECKITEM_NAME = adjustCheckPoint.CHECKITEM_NAME,
                    ESTIMATED_ENDDATE = adjustCheckPoint.ORI_ESTIMATED_ENDDATE.ToTwDateString(),
                    ESTIMATED_ENDDATE_ADJ = adjustCheckPoint.ESTIMATED_ENDDATE.ToTwDateString(),
                });
            }
