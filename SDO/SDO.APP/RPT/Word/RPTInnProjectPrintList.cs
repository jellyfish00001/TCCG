...
{
    public class RPTInnProjectPrintList : WordBuilder
    {
        private readonly IInnProjectService innProjectService;

        public RPTInnProjectPrintList(IComponentContext coms) : base(coms)
        {
        ...
        protected override async Task<bool> MakeContent()
        {
            ...
            foreach (var projectNO in Parameter.PROJECT_NO_DATA)
            {
                ...
                await CreateService<RPTInnProjectPrintList>(objData, "InnProjectFillBasic").MakeContent();
